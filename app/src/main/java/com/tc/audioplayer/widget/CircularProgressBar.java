...
public class CircularProgressBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float r = rectF.height() / 4;
        float x1 = (float) (x - Math.sqrt(3) * r / 4);
        float y1 = y + (float) (Math.sqrt(3) * r / 2);
        float x2 = x1;
        float y2 = (float) (y - Math.sqrt(3) * r / 2);
        if (isPlaying) {
            float x3 = (float) (x + Math.sqrt(3) * r / 4);
            float y3 = y1;
            float x4 = x3;
            float y4 = y2;
            canvas.drawLine(x1, y1, x2, y2, backgroundPaint);
            canvas.drawLine(x3, y3, x4, y4, backgroundPaint);
        } else {
            float x3 = x + r;
            float y3 = y;
            path.reset();
